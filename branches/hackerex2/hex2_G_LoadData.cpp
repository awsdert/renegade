...
void hexDB::loadData( int doAct, Text name )
{
	/**
		...
		@param inMode
		determines which file set is retrieved and what tData to expect, in some cases a file is not loaded but tData from a local object instead
		@param name
		is which object to keep hold of when loading tData, in some cases this is ignored. In the event a name cannot be found the default object will be used.
	**/
...
bool LoadData( hexDB& db, int& doAct, TxtA& tData, Text& pNow, Text& pTmp, Text& name, bool& isFileTmp, bool& isTempTmp )
{
	int  m = 0, atMode = db.mode[ db.tmpMode ];
	bool b = true, isSame = ( db.nowP[ atMode ] == db.oldP[ atMode ] ), addObj = ( isSame && db.getNowN( db.tmpMode ) != getGlobalName() );
	switch ( db.tmpMode )
	{
	case HEX_LIST_THEME:
