...
void G::UpdateTheme( Text name, int size )
{
	...
	int  tia[ iCount ];
	tia[   i ] = HexMain_B_ID;	tha[ i ] = wxT("manager");
	tia[ ++i ] = HexList_B_ID;	tha[ i ] = wxT("db");
	tia[ ++i ] = HexSave_B_ID;	tha[ i ] = wxT("save");
	/*
	...
	//*/
	tia[ ++i ] = HexNew_B_ID;	tha[ i ] = wxT("db_add");
	tia[ ++i ] = HexDel_B_ID;	tha[ i ] = wxT("db_del");
	tia[ ++i ] = HexGet_B_ID;	tha[ i ] = wxT("db_get");
	tia[ ++i ] = HexSet_B_ID;	tha[ i ] = wxT("db_set");
	for ( i = 0; i < iCount; ++i )
	{
		ico = theme + tha[ i ] + ext;
		if ( !wxFileExists( ico ) )
